...
/**
 * Top App Bar with title and account avatar
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun HomeTopAppBar(
    onAccountClick: () -> Unit, modifier: Modifier = Modifier
) {
    TopAppBar(
        title = {
        Text(
            text = "Linky", style = MaterialTheme.typography.headlineSmall
        )
    }, actions = {
    ...
    ), windowInsets = WindowInsets.statusBars, modifier = modifier
    )
}
